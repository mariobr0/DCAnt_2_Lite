...
### Ручная проверка
- [ ] Запустить ObserveOnly и убедиться, что торговые команды не отправляются.
- [ ] Запустить PlaceAndObserve с неисполняемой ценой.
- [ ] Скопировать TARGET_MARKER_CREATED из журнала.
- [ ] Убедиться, что ордер появился в Quantower.
- [ ] Остановить диагностическую Strategy.
- [ ] Запустить CancelAndObserve с сохраненным TargetMarker.
- [ ] Убедиться, что отправлен ровно один Cancel.
- [ ] Проверить итоговое состояние ордера вручную.
- [ ] Проверить отсутствие открытой позиции.
- [ ] Проверить, остался ли активный test-order.

### Финальная проверка
- [ ] Запустить применимые автоматические тесты (Core-тесты должны проходить).
- [ ] Выполнить `git diff --check`.
- [ ] Убедиться, что EngineLoop и TradeCycle не изменены.
- [ ] Убедиться, что не добавлены SQLite, Outbox и QuantowerAdapter.
